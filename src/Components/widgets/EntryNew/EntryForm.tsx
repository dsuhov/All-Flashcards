...
import {
  cleanEntriesFields,
  validateEntriesFields,
  extendDefaultEntryContent,
} from './utils';
import {
  EntryNewProps,
...
export const EntryForm = (props: EntryNewProps) => {
  const {
    onSave,
    onCancel,
    showAddEntryBtn = true,
    defaultEntryContent,
    pending = false,
  } = props;
  ...
  const [entryContents, setEntryContents] = useState<NewEntryContentSheme[]>(
    defaultEntryContent
      ? [extendDefaultEntryContent(defaultEntryContent)]
      : [
          {
            entryText: '',
            transcription: '',
